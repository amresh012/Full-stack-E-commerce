...
const Orders = () => {
  ...
  const columns = [
        ...
        return (
          ...
          >
            {orderStates.map((orderstat) => (
              <option key={orderstat} value={orderstat}>
                {orderstat}
              </option>
            ))}
          </select>
    ...
    },
    {
      ...
      cell: ({ row }) => {
        return (
          <div className="flex w-full justify-around  cursor-pointer ">
            <div
              className=""
              title="delete order"
            ...
            <div
              id="invoice-content"
              onClick={() => {
                fetchInvoice(row.original?.invoiceNo);
              }}
  ...
  const fetchInvoice = async (invoiceNo) => {
    try {
      const response = await axios.get(
        `${base_url}order/invoice/${invoiceNo}`,
        config
      );
  ...
  return (
    <>
      <Toaster />
      {/* table col */}
      <div className=" border m-2 min-h-[70vh]  rounded-md ">
        <div className="border-b p-4 flex items-center justify-between">
          <h1 className="text-gray-700 font-medium uppercase">Order information</h1>
          <div className="flex flex-col ">
            <div className="border border-gray-700 flex items-center justify-between rounded-md p-2">
              <input
                type="search"
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                className=" outline-none px-2 placeholder:px-2 "
        ...
        <div className="flex p-4 items-start justify-start gap-4">
              ...
              <div key={item.id}>
                ...
                <Select
                  onChange={(selectedOption) =>
                    sortByStatus(selectedOption.value)
                  } // Call sortByStatus with selected status
                  options={[
                    { value: "", label: "All" },
