    ...
    },
    {
      ...
      cell: ({ row }) => (
        <List className="flex items-center gap-2 justify-center cursor-pointer">
          <div
            onClick={() => fetchQueryDetails(row.original._id)}
  ...
  const fetchDetails = async () => {
    try {
      ...
      const response = await fetch(
        `https://crmkfsbackend.deepmart.shop/api/support/get-all-support`,
        {
          method: "GET",
          headers: {
    ...
    <div>
              ...
              <div className="text-lg" key={index}>
                ...
                <div className="mb-1 flex items-center gap-x-2">
                  ...
                  <span className="font-bold">
                    {detail?.assigned.name || "Not Assigned"}
                  </span>
                </div>
                <div className="mb-1 flex items-center gap-x-2">
                  <span className="font-semibold">Status:</span>
                  <span className="bg-green-200 text-xs p-1 uppercase rounded-md text-green-500">
                    {detail?.status}
                  </span>
                </div>
              </div>
      ...
      <div className="border m-2 rounded-md  flex flex-col items-center justify-normal">
        <div className="p-4 border-b w-full">
          <h1 className="text-gray-700 font-medium uppercase">Contact List</h1>
        </div>
