    ...
    <footer className=" bg-black w-full text-white flex flex-col">
      ...
      <div className="top-footer flex lg:flex-row  flex-col items-start around  border-b-2 lg:p-2 lg:m-4">
        <div className="p-4 space-y-4 child-1 lg:w-1/3 w-full  ">
          <Logo />
          <p className="">
            KFS FITNESS- Adding fuel to every fitness fanatic’s passion with the
            best equipment. Experts in setting home/commercial Gyms & CrossFit
            boxes PAN India.
          </p>
          <p className="">
          ...
          <div className="social-media flex gap-8 ">
            {socialMedia.map((media) => (
              <Link to={media.link} key={media.id}>
                <div className="cursor-pointer hover:bg-white p-2 rounded-md hover:scale-105 duration-300 hover:text-black ">
        ...
        <div className="p-4 flex items-start justify-start flex-col">
          ...
          <ul className="lg:h-56 cursor-pointer flex flex-col flex-wrap">
            {prod.map((item, index) => (
              <li className="p-2 hover:pl-2  hover:underline" key={index}>
                {item}
              </li>
            ))}
          </ul>
          ...
          <ul className="lg:h-[200px] cursor-pointer flex flex-col flex-wrap">
            {polocies.map((item) => (
              <Link to={item.route} key={item.id}>
                <li className="p-2 hover:pl-2  hover:underline" key={item.id}>
                  {item.name}
                </li>
              </Link>
            ))}
